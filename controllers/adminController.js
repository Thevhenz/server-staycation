...
module.exports = {
  ...
  showEditItem: async (req, res) => {
    try {
      ...
      const category = await Category.find();
      const alertMessage = req.flash("alertMessage");
      const alerStatus = req.flash("alertStatus");
      const alert = { message: alertMessage, status: alerStatus };
      res.render("admin/item/view_item", {
    ...
    try {
      ...
      for (let i = 0; i < item.imageId.length; i++) {
        Image.findOne({ _id: item.imageId[i]._id })
          ...
          .catch((error) => {
            req.flash("alertMessage", `${error_message}`);
            req.flash("alertStatus", "danger");
            res.redirect("/admin/item");
          });
      }
  ...
  editActivity: async (req, res) => {
    ...
    try {
      ...
      if (req.file == undefined) {
        ...
        await activity.save();
        req.flash("alertMessage", "Success Update Image Activity");
        req.flash("alertStatus", "success");
        res.redirect(`/admin/item/show-detail-item/${itemId}`);
      }
    } catch (error) {
  ...
  actionAccepted: async (req, res) => {
    ...
    try {
      ...
      await booking.save();
      req.flash("alertMessage", "Payment Confirmation Accepted");
